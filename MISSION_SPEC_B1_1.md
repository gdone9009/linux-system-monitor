# 🛡️ [B1-1] 시스템 관제 자동화 및 보안 구축 미션 명세서
...
| **구분** | Linux와 OS |
| **학습시간** | 40시간 |
| **미션 주제** | **컴퓨터가 알아서 자기 상태를 점검하게 만들기** |

---

## 1. 미션 소개 (Mission Overview)
...
다음 **2가지 산출물**을 제출해야 합니다.

### [산출물 1] 요구사항 수행 내역서 (문서 1개)
1. **수행 내역**: 설정/명령어 기록 (SSH 포트, 방화벽 규칙, 계정/그룹/ACL, 디렉토리/권한, 환경 변수, cron 등록 등)
2. **필수 증거 자료 체크리스트 (8개 필수 항목)**
...
## 3. 과제 목표 (Learning Objectives)

이 과제를 마친 후, 학습자는 아래를 스스로 설명할 수 있어야 합니다.

1. **SSH 포트 변경과 Root 원격 접속 차단**이 왜 기본 보안에 해당하는지 설명할 수 있다.
2. **UFW 또는 firewalld** 중 하나를 선택해 “필요 포트만 허용”하는 방화벽 정책을 구성하고 검증할 수 있다.
3. **역할 기반 계정/그룹과 ACL**을 통해 “공유 디렉토리”와 “보안 디렉토리”를 분리하는 이유를 설명할 수 있다.
4. **환경 변수(`AGENT_HOME` 등)**로 실행 환경을 고정하는 이유와 검증 방법을 설명할 수 있다.
5. **쉘 스크립트**로 프로세스/포트/리소스 상태를 수집하고, 로그로 남겨 운영 문제를 추적하는 흐름을 설명할 수 있다.
6. **crontab**으로 모니터링을 주기 실행시키고, **로그 보존 정책(압축/삭제)**이 왜 필요한지 설명할 수 있다.
...
* **Root 원격 로그인(`PermitRootLogin no`)**을 차단한다.
* *확인 방법(예시)*:
...
* *확인 방법(예시)*:
  * UFW 선택 시: `sudo ufw status verbose`
...
#### 2) 생성 그룹
* **`agent-common`**: `admin`, `dev`, `test` 모두 포함
* **`agent-core`**: `admin`, `dev` 포함

#### 3) 디렉토리 구조 (`AGENT_HOME` 기준)
```text
$AGENT_HOME/
├── upload_files/       # 공용 협업 저장소
├── api_keys/           # 핵심 보안 키 저장소
└── bin/                # 실행 스크립트 저장소
/var/log/agent-app/     # 시스템 운영 로그 저장소
```

#### 4) 접근 권한 (핵심 정책)
* **`upload_files`**: group=`agent-common`, **R/W 가능 (775)**
* **`api_keys` 및 `/var/log/agent-app`**: group=`agent-core` **ONLY, R/W 가능 (770)**
* *확인 방법(예시)*:
  * `id agent-admin` / `id agent-dev` / `id agent-test`
  * `ls -ld` 및 `getfacl`(사용 시)로 소유/권한 확인

---

### 4.3 애플리케이션 실행 환경 구성 (제공 Python 앱)

#### 1) 환경 변수 정의
* `AGENT_HOME`: 예) `/home/agent-admin/agent-app` (또는 사용자 홈 기준)
* `AGENT_PORT`: `15034`
...
#### 3) 앱 실행 및 성공 기준
* **일반 계정으로 실행 (루트 실행 엄격 금지)**
* Boot Sequence 5단계가 모두 `[OK]`로 출력되고, 마지막에 **“Agent READY”**가 출력되어야 한다.
* 앱이 `0.0.0.0:15034`로 **LISTEN 상태**가 되어야 한다.
* *(참고: 앱 종료는 `Ctrl+C`로 수행)*
...
* 그룹: `agent-core`
* **권한: `750` (`rwxr-x---`)**
* cron 실행 계정: `agent-admin` (`agent-admin`은 `agent-core` 그룹에 포함되어 실행 가능해야 함)
...
#### 7) 로그 파일 용량 관리
* `monitor.log`가 커지면 **최대 10MB / 10개 파일 유지** (방법 자유: logrotate 사용 또는 스크립트 로직 구현)
...
### 6.1 개발 환경
* **Ubuntu 22.04 LTS / 24.04 LTS** 또는 동등 리눅스 환경
* 이전 미션에서 구성한 Linux 실습 환경(OrbStack VM / 컨테이너 / VM)을 그대로 사용 권장

### 7.1 제약 사항
* **구현 언어/도구**: 자동화 스크립트는 **Bash로만 작성**한다 (Python 등으로 대체 금지).
* **최소 권한 준수**: 필요한 경우에만 `sudo` 사용 (가능한 일반 계정으로 진행).
* **제공 애플리케이션**: 제공된 Python 앱은 “실행 대상”이며, 과제의 핵심은 **관제/자동화 스크립트 구현**이다.

---

## 8. 결과 예시 (Test Case Reference)

> 아래는 정답이 아니라 참고 예시입니다. 실제 문구와 구성은 세부 구현에 따라 다를 수 있습니다.
...
### 8.4 (보너스 수행 시) `report.sh` 콘솔 출력 예시
```text
====== STATISTICS REPORT ======
[CPU]
Average : 21.4%
...
## 9. 데이터 파일 안내 (Data Files)

* **배포 파일**: `agent-app.zip`
* **지원 아키텍처**:
  * `agent-app-linux-x86` (Intel/AMD x86_64 리눅스)
  * `agent-app-linux-arm64` (ARM / Apple Silicon 리눅스)
